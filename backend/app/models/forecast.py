...
class DemoForecast(Base):
    __tablename__ = "demo_forecasts"

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=generate_uuid)
    user_id: Mapped[str] = mapped_column(String(36), ForeignKey("users.id"), nullable=False, index=True)
    horizon: Mapped[str] = mapped_column(String(20), nullable=False)  # day, week, month, quarter, year
    forecast_date: Mapped[date] = mapped_column(Date, nullable=False)
    predicted_amount: Mapped[float] = mapped_column(Float, nullable=False)
    lower_bound: Mapped[float] = mapped_column(Float, nullable=True)
    upper_bound: Mapped[float] = mapped_column(Float, nullable=True)
    ...
    # Relationships
    user = relationship("User", back_populates="demo_forecasts", lazy="selectin")

    def __repr__(self) -> str:
        return f"<DemoForecast {self.horizon} ${self.predicted_amount:.2f}>"


class AWSForecast(Base):
    __tablename__ = "aws_forecasts"
    ...
    # Relationships
    user = relationship("User", back_populates="aws_forecasts", lazy="selectin")

    def __repr__(self) -> str:
        return f"<AWSForecast {self.horizon} ${self.predicted_amount:.2f}>"
